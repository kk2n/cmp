...
import { filterArr, findObj } from '../../array'
...
export default class extends Component {
  constructor(props) {
    ...
    //防抖
    this.search = debounce(this.search, props.debounceTime || 800)
  }
  static propTypes = {
    optionDisabled: PropTypes.array,
  ...
  getVT = () => {
    ...
    if (this.props.vt) {
      ...
      let aData = this.props.data?.length
        ? this.props.data
        : this.props.beforeData?.length
  ...
  render() {
    ...
    return (
      <Select
        ...
        labelInValue
        mode="multiple"
        disabled={this.props.loadingDisabled && this.state.fetching}
      ...
      >
        {[...this.props.beforeData, ...(this.props.data || []), ...(data || []), ...this.props.afterData].map(
          (d, dd) => {
            return (
              d[vt[0]] !== undefined && (
                <Option
                  ...
                  value={d[vt[0]]}
                  title={d[vt[1]]}
                  disabled={this.props.optionDisabled?.includes(d[vt[0]])}
                >
                  {d[vt[1]] +
